...
import actividades.actividadbackend.service.soproduc_service;
...
public class soproduc_controller {
	
	@Autowired
	private soproduc_service service;
	
	@ApiOperation(value="recibe todos los productos", 
			notes="Muestra todos los elementos de la tabla soproduc,"
	...
	@ApiOperation(value="recibe un producto de la tabla soproduc", 
			notes="Muestra un producto en función del id de busqueda ingresado en la petición,"
	...
	@ApiResponses(value = { @ApiResponse(code=HttpServletResponse.SC_BAD_REQUEST,message="Faltan Elementos por llenar"),
			@ApiResponse(code=HttpServletResponse.SC_INTERNAL_SERVER_ERROR,message="Algo salió mal")})
	public ResponseEntity <Optional<Soproduc>>buscar(@PathVariable ("id") int id){
		return service.buscarproductos(id);
		
	}
	
	@ApiOperation(value="Ingresa un nuevo elemento de producto a la tabla Soproduc", 
				notes="recibe un objeto de tipo soproduc, que tiene relación con la entidad para validar"
	...
	@ApiResponses(value = { @ApiResponse(code=HttpServletResponse.SC_CREATED,message="Elemento creado"),
							...
							@ApiResponse(code=HttpServletResponse.SC_INTERNAL_SERVER_ERROR,message="Algo salió mal"),})
	public ResponseEntity <Soproduc> insertar(@RequestBody Soproduc entity) {
		return service.insertarproductos(entity);
	}
	
	@ApiOperation(value="Actualiza los valores de un producto a la tabla Soproduc",
				notes="recibe los datos de una entidad soprouc, con la finalidad de evaluar los datos,"
	...
	@ApiResponses(value = { @ApiResponse(code=HttpServletResponse.SC_CREATED,message="Elemento Actualizado"),
			@ApiResponse(code=HttpServletResponse.SC_BAD_REQUEST,message="Faltan Elementos por llenar"),
			@ApiResponse(code=HttpServletResponse.SC_INTERNAL_SERVER_ERROR,message="Algo salió mal"),})
	public ResponseEntity<Soproduc> actualizar(@PathVariable ("id") int id, @RequestBody Soproduc entity) {
		return service.actualizarproductos(id, entity);
	}
	...
	public  ResponseEntity <Soproduc> eliminar(@PathVariable ("id") int id) {
		try {
			service.eliminarproductos(id);
			return new ResponseEntity<Soproduc>(HttpStatus.OK);
		}catch (Exception e) {
